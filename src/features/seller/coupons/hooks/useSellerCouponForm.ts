...
import type { SellerCouponFormErrors, SellerCouponFormValues } from '../types/sellerCoupon';
...
export function useSellerCouponForm(userId?: string, couponId?: string) {

  const isEditMode = Boolean(couponId);

  const [values, setValues] = useState<SellerCouponFormValues>(createEmptySellerCouponFormValues());

  const [errors, setErrors] = useState<SellerCouponFormErrors>({});

  const [isLoading, setIsLoading] = useState(Boolean(isEditMode && couponId));
